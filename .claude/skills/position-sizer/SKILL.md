...
4. **Stop distance** — exactly one of:
   ...
   - `stop_distance` (distance in price units, e.g. `"28.59"` for $28.59 on XAUUSD). Used directly by `price-action` hand-offs — pass through without conversion.
...
If the user's defaults are stored in `~/.trading-agent-skills/config.toml`, default `risk_pct` to `config.risk.per_trade_max_pct` (typically 1.0).
...
The bundle shape is documented in `src/trading_agent_skills/cli/size.py`. Use Decimal-as-string for all money / price / volume fields — that's mt5-mcp's serialisation contract and the CLI rejects floats.
