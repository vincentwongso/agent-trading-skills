...
- **Currency vs ticker tagging.** Finnhub returns ticker symbols in `related`; Marketaux returns `entities` with symbols; ForexNews returns currency codes (`["USD", "EUR"]`). The orchestrator handles all three by matching on either tickers OR currency-of-interest derived from the symbol's broker meta. Don't try to canonicalise upstream.
