# CFD Trading Skills — High-Level Plan

**Status:** Implementation-ready. Standalone document, intended to be moved into a new repository dedicated to Claude Code skills for CFD day trading.
...
- **Skills directory layout** (target repo):
  ...
  .claude/skills/
    ...
    _shared/
      ...
      config_io.py           # ~/.cfd-skills/config.toml read/write + first-run flow
...
## Skill 1 — `cfd-position-sizer`

**Goal:** Compute correct lot size for a CFD trade given account equity, risk %, stop distance, and symbol — accounting for contract size, leverage, margin consumption, and overnight swap exposure.
...
- Swap section omits gracefully when symbol has zero swap (e.g. some crypto CFDs)
...
**Dependencies:** `mt5-market-data` (`get_account_info`, `get_history`, `get_positions`, `get_quote`, `get_rates` for indicator context), local `daily_state.json`, `~/.cfd-skills/config.toml`.
...
- Per-check breakdown:
  ...
  - **Spread state:** current spread vs. baseline (stored per symbol in `~/.cfd-skills/spread_baseline.json`, EWMA-updated on each call)
...
**Dependencies:** Local file `~/.cfd-skills/journal.jsonl`. `mt5-market-data` (`get_history` for auto-populate).
...
3. Fire requests to all 3 news APIs + Calix in parallel via `asyncio.gather`; cache 60s on disk under `~/.cfd-skills/news_cache/`
...
On any skill's first invocation, if `~/.cfd-skills/config.toml` does not exist, the skill:
...
mt5-mcp enhancements PR (SymbolInfo enrichment + get_rates + calc_margin)
              │
              ▼
cfd-position-sizer ──────── (independent, build first after mt5-mcp PR merges)
                                    │
                                    ▼
...
2. `cfd-position-sizer` — fastest to build, hits every trade, immediate ROI
...
Then proceed to `cfd-position-sizer` as the first skill build.
