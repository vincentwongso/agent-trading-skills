...
**Goal:** Build the `price-action` skill — a hybrid classical + ICT structural reader that emits 0–3 ranked setup candidates plus full structure (pivots, S/R, FVGs, OBs, liquidity pools, EMA stack, regime) from MT5 OHLC bars, binding to existing `pre-trade-checklist` and `position-sizer` skills.

**Architecture:** New sub-package `src/trading_agent_skills/price_action/` (first sub-package in repo) with deterministic Python core (9 detectors + structure layer + scoring) and an LLM layer in `SKILL.md` that picks `selected_setup_id` and writes `selection_rationale`. Same JSON-stdin → pure-function → JSON-stdout shim pattern as the existing four skills.

**Tech Stack:** Python 3.11+, Decimal-typed throughout, pytest, `Bar` reused from `trading_agent_skills.indicators`, no new third-party deps. Tests use hand-rolled fixture factories — no mocks.

**Spec:** `docs/superpowers/specs/2026-04-29-price-action-skill-design.md`
...
- All numeric inputs/outputs are `Decimal`; floats are rejected at boundaries via `trading_agent_skills.decimal_io.D()`.
...
**Files:**
- Create: `src/trading_agent_skills/price_action/__init__.py`
- Create: `src/trading_agent_skills/price_action/bars.py`
- Create: `src/trading_agent_skills/price_action/detectors/__init__.py` (empty placeholder for now)
- Create: `tests/test_price_action_bars.py`

- [ ] **Step 1: Create empty package files**

```bash
touch src/trading_agent_skills/price_action/__init__.py
touch src/trading_agent_skills/price_action/detectors/__init__.py
```
...
Expected: collection error (`ModuleNotFoundError: No module named 'trading_agent_skills.price_action.bars'`).

- [ ] **Step 4: Implement `bars.py`**

`src/trading_agent_skills/price_action/bars.py`:

```python
"""MTF bar bundle — thin wrapper around per-timeframe ``indicators.Bar`` lists.

Reuses ``trading_agent_skills.indicators.Bar`` (do not create a parallel Bar class —
...
from trading_agent_skills.indicators import Bar, bars_from_mcp
...
```bash
git add src/trading_agent_skills/price_action/__init__.py src/trading_agent_skills/price_action/detectors/__init__.py src/trading_agent_skills/price_action/bars.py tests/test_price_action_bars.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/pivots.py`
...
`src/trading_agent_skills/price_action/pivots.py`:
...
```bash
git add src/trading_agent_skills/price_action/pivots.py tests/test_price_action_pivots.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/structure.py`
...
`src/trading_agent_skills/price_action/structure.py`:
...
```bash
git add src/trading_agent_skills/price_action/structure.py tests/test_price_action_structure.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/fvg.py`
...
`src/trading_agent_skills/price_action/fvg.py`:
...
```bash
git add src/trading_agent_skills/price_action/fvg.py tests/test_price_action_fvg.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/order_block.py`
...
`src/trading_agent_skills/price_action/order_block.py`:
...
```bash
git add src/trading_agent_skills/price_action/order_block.py tests/test_price_action_order_block.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/liquidity.py`
...
`src/trading_agent_skills/price_action/liquidity.py`:
...
```bash
git add src/trading_agent_skills/price_action/liquidity.py tests/test_price_action_liquidity.py
...
**Files:**
- Modify: `src/trading_agent_skills/price_action/detectors/__init__.py`
...
`src/trading_agent_skills/price_action/detectors/__init__.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/__init__.py tests/test_price_action_candidate.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/context.py`
...
`src/trading_agent_skills/price_action/context.py`:
...
from trading_agent_skills.indicators import Bar, InsufficientBars, atr, ema
from trading_agent_skills.price_action.bars import MTFBars
from trading_agent_skills.price_action.fvg import FVG, detect_fvgs
from trading_agent_skills.price_action.liquidity import LiquidityPool, derive_liquidity_pools
from trading_agent_skills.price_action.order_block import OrderBlock, detect_order_blocks
from trading_agent_skills.price_action.pivots import Pivot, classify_sequence, detect_pivots
from trading_agent_skills.price_action.structure import (
    MTFAlignment,
    RegimeKind,
    SRLevel,
...
```bash
git add src/trading_agent_skills/price_action/context.py tests/test_price_action_context.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/pullback_ema.py`
...
`src/trading_agent_skills/price_action/detectors/pullback_ema.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/pullback_ema.py tests/test_detector_pullback_ema.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/sr_bounce.py`
...
`src/trading_agent_skills/price_action/detectors/sr_bounce.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/sr_bounce.py tests/test_detector_sr_bounce.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/pin_bar.py`
...
`src/trading_agent_skills/price_action/detectors/pin_bar.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/pin_bar.py tests/test_detector_pin_bar.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/engulfing.py`
...
`src/trading_agent_skills/price_action/detectors/engulfing.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/engulfing.py tests/test_detector_engulfing.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/range_break_retest.py`
...
from trading_agent_skills.price_action.bars import MTFBars
from trading_agent_skills.price_action.context import build_context
from trading_agent_skills.price_action.detectors.range_break_retest import detect
...
`src/trading_agent_skills/price_action/detectors/range_break_retest.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/range_break_retest.py tests/test_detector_range_break_retest.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/fvg_fill.py`
...
`src/trading_agent_skills/price_action/detectors/fvg_fill.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/fvg_fill.py tests/test_detector_fvg_fill.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/ob_retest.py`
...
`src/trading_agent_skills/price_action/detectors/ob_retest.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/ob_retest.py tests/test_detector_ob_retest.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/liq_sweep.py`
...
from trading_agent_skills.price_action.bars import MTFBars
from trading_agent_skills.price_action.context import build_context
from trading_agent_skills.price_action.detectors.liq_sweep import detect
...
`src/trading_agent_skills/price_action/detectors/liq_sweep.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/liq_sweep.py tests/test_detector_liq_sweep.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/detectors/bos_pullback.py`
...
from trading_agent_skills.price_action.bars import MTFBars
from trading_agent_skills.price_action.context import build_context
from trading_agent_skills.price_action.detectors.bos_pullback import detect
...
`src/trading_agent_skills/price_action/detectors/bos_pullback.py`:
...
```bash
git add src/trading_agent_skills/price_action/detectors/bos_pullback.py tests/test_detector_bos_pullback.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/scoring.py`
...
`src/trading_agent_skills/price_action/scoring.py`:
...
``~/.trading-agent-skills/config.toml``.
"""
...
from trading_agent_skills.price_action.detectors import CandidateSetup
from trading_agent_skills.price_action.structure import MTFAlignment
...
```bash
git add src/trading_agent_skills/price_action/scoring.py tests/test_price_action_scoring.py
...
**Files:**
- Create: `src/trading_agent_skills/price_action/schema.py`
- Create: `src/trading_agent_skills/price_action/scan.py`
- Modify: `src/trading_agent_skills/price_action/__init__.py` (add re-exports)
...
`src/trading_agent_skills/price_action/schema.py`:

```python
"""Output schema for price-action scans."""
...
from trading_agent_skills.price_action.pivots import Pivot
from trading_agent_skills.price_action.structure import MTFAlignment, RegimeKind, SRLevel
...
`src/trading_agent_skills/price_action/scan.py`:
...
from trading_agent_skills.price_action.bars import MTFBars, timeframe_minutes
from trading_agent_skills.price_action.context import ScanContext, build_context
from trading_agent_skills.price_action.detectors import CandidateSetup
from trading_agent_skills.price_action.detectors import bos_pullback as _bos
from trading_agent_skills.price_action.detectors import engulfing as _eng
from trading_agent_skills.price_action.detectors import fvg_fill as _fvg
from trading_agent_skills.price_action.detectors import liq_sweep as _liq
from trading_agent_skills.price_action.detectors import ob_retest as _ob
from trading_agent_skills.price_action.detectors import pin_bar as _pin
from trading_agent_skills.price_action.detectors import pullback_ema as _pull
from trading_agent_skills.price_action.detectors import range_break_retest as _rng
from trading_agent_skills.price_action.detectors import sr_bounce as _srb
from trading_agent_skills.price_action.schema import (
    EmaStackSnapshot,
    SCHEMA_VERSION,
    ScanResult,
)
...
`src/trading_agent_skills/price_action/__init__.py`:
...
```bash
git add src/trading_agent_skills/price_action/schema.py src/trading_agent_skills/price_action/scan.py src/trading_agent_skills/price_action/__init__.py tests/test_price_action_scan.py
...
**Files:**
- Create: `src/trading_agent_skills/cli/price_action.py`
...
`src/trading_agent_skills/cli/price_action.py`:
...
```toml
[project.scripts]
trading-agent-skills-size = "trading_agent_skills.cli.size:main"
trading-agent-skills-journal = "trading_agent_skills.cli.journal:main"
trading-agent-skills-guardian = "trading_agent_skills.cli.guardian:main"
trading-agent-skills-checklist = "trading_agent_skills.cli.checklist:main"
trading-agent-skills-news = "trading_agent_skills.cli.news:main"
trading-agent-skills-price-action = "trading_agent_skills.cli.price_action:main"
```
...
```bash
echo '{"symbol":"X","mode":"swing","timeframes":["H1"],"as_of":"2026-04-05T00:00:00+00:00","current_quote":{"bid":"100","ask":"100","time":"x"},"symbol_meta":{"tick_size":"0.01","digits":2,"contract_size":"100","trade_mode":"full"},"rates":{"H1":[]}}' | trading-agent-skills-price-action
```
...
```bash
git add src/trading_agent_skills/cli/price_action.py pyproject.toml tests/test_cli_price_action.py
git commit -m "feat(price-action): CLI shim + trading-agent-skills-price-action entry point"
```
...
**Files:**
- Create: `.claude/skills/price-action/SKILL.md`
- Create: `.claude/skills/price-action/scripts/price_action.py`
- Modify: `CLAUDE.md` (status section + layout entry)

- [ ] **Step 1: Create the script shim**

`.claude/skills/price-action/scripts/price_action.py`:
...
`.claude/skills/price-action/SKILL.md`:
...
- For `position-sizer`: pass `{symbol, side, stop_distance}` (plus the user's account/quote bundle).
...
In the entry-point list at the bottom of pyproject.toml documentation, ensure `trading-agent-skills-price-action` is mentioned.
...
JSON
trading-agent-skills-price-action --input /tmp/pa_smoke.json
```
...
```bash
git add .claude/skills/price-action/ CLAUDE.md
git commit -m "feat: price-action skill (skill #5)"
git push
```
...
- [ ] `Bar` from `trading_agent_skills.indicators` reused (no parallel Bar class).
...
- [ ] Entry point `trading-agent-skills-price-action` registered + reinstalled.
