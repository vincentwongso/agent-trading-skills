# cfd-price-action skill — design

**Status:** draft, awaiting user review
**Date:** 2026-04-29
**Author:** Claude (brainstorming session with Vincent)
**Sibling skills already shipped:** cfd-position-sizer, trade-journal, daily-risk-guardian, pre-trade-checklist, session-news-brief

---

## 1. Purpose

Provide a structured price-action read of a CFD/FX/index symbol from MT5 OHLC bars, emitting up to three ranked setup candidates plus the full underlying structure (pivots, S/R, FVGs, order blocks, liquidity pools, EMA stack, regime per timeframe). The output is consumed downstream by `pre-trade-checklist` and `cfd-position-sizer` for a coherent "is this a tradeable idea, and if so what size" workflow.
...
Frontmatter `description` for `.claude/skills/cfd-price-action/SKILL.md`:
...
- **Python layer** (`src/cfd_skills/price_action/`): deterministic, Decimal-typed, no I/O at the package boundary. Detects structurally-valid candidates only — cannot hallucinate FVGs/OBs that don't satisfy formal definitions.
...
This is the first sub-package in `cfd_skills/`. Justification: 9 detectors + 5 structure modules + scoring + schema is too much for a single file. Existing skills with single-file modules have ~300 lines; this would be ~1500. A sub-package keeps each detector independently testable.
...
Agent in SKILL.md flow
  ...
  ↓
JSON bundle on stdin →  python -m cfd_skills.cli.price_action  → JSON on stdout
                              │
...
LLM in SKILL.md flow:
  ...
  - optional hand-off prompt: "want me to run pre-trade-checklist + cfd-position-sizer for this?"
```
...
`config` block is optional; if omitted, the CLI reads from `~/.cfd-skills/config.toml` and falls back to shipped defaults.

## 6. Detector menu (v1: 9 detectors)

Each detector lives in `cfd_skills/price_action/detectors/<name>.py` and exposes `detect(bundle: ScanBundle, structure: Structure) -> list[CandidateSetup]`.
...
Default weights (overridable in `~/.cfd-skills/config.toml`):
...
| `cfd-position-sizer` | `symbol`, `selected_setup.side`, `selected_setup.stop_distance` |
...
src/cfd_skills/cli/price_action.py   # JSON-stdin / JSON-stdout shim, calls scan()

.claude/skills/cfd-price-action/
  SKILL.md
  scripts/price_action.py
```

`pyproject.toml` entry point: `cfd-skills-price-action = "cfd_skills.cli.price_action:main"`.

## 13. Configuration

`~/.cfd-skills/config.toml` (existing, optional):
...
All values Decimal-as-string where applicable. Defaults shipped in `cfd_skills.config_io`.
...
4. `pre-trade-checklist` and `cfd-position-sizer` run successfully when piped the selected setup's `{symbol, side, suggested_entry, invalidation, stop_distance}`.
