"""CLI wrapper for ``trading_agent_skills.news_brief.build``.
...
def _is_equity_like_ticker(symbol: str) -> bool:
    ...
    (AAPL, MSFT, SPY, ^GSPC...). Broker symbol names like ``XAUUSD.z`` or
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="trading-agent-skills-news")
    parser.add_argument(
        "--input", "-i", default="-",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "~/.trading-agent-skills/.env then ./.env in the working directory. "
    ...
    if args.env_file is not None:
        ...
        for candidate in (Path.home() / ".trading-agent-skills" / ".env", Path.cwd() / ".env"):
