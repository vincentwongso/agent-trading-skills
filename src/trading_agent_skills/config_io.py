"""Read/write ``~/.trading-agent-skills/config.toml``.
...
DEFAULT_CONFIG_PATH = Path.home() / ".trading-agent-skills" / "config.toml"
...
def load_config(
    ...
    *,
    ...
    """Load config from ``path`` (defaults to ``~/.trading-agent-skills/config.toml``).
...
def write_config(cfg: Config, path: Path | None = None) -> Path:
    """Write config to ``path`` (defaults to ``~/.trading-agent-skills/config.toml``).
